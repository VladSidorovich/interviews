...
## Группа 1: Event Loop & Асинхронность
...
### 5. ⭐⭐⭐ Как возможна гонка данных (race condition) в асинхронном программировании?
...
### 7. ⭐⭐⭐⭐ Почему у Event Loop есть фазы? Чем отличаются микротаски и макротаски?
...
## Группа 2: Потоки и Backpressure

### 8. ⭐⭐⭐ Как обрабатывать CPU-интенсивные задачи в Node.js?

> Q2 закрывает sync I/O — этот вопрос про CPU-bound блокировку
...
### 9. ⭐⭐⭐ Как могут утечь все соединения из пула подключений к БД?
...
### 10. ⭐⭐⭐⭐ JSON сериализация/десериализация блокирует поток — что делать?
...
## Группа 3: Кластеризация и память

### 12. ⭐ Какие Web API появились в Node.js в последнее время и зачем их добавляют?

**🟡 Junior**
Знает что добавили нативный `fetch` и теперь не нужен `axios` / `node-fetch`.

**🟠 Middle**
Называет несколько API с версиями:
...
| `AbortController` | 15+ |
| `structuredClone` | 17+ |
| `URL`, `URLSearchParams` | 10+ |
| `FormData`, `Headers`, `Response` | 18+ |
| `ReadableStream` / `WritableStream` | 16+ |
| `crypto.subtle` (WebCrypto) | 15+ |

**🔴 Senior**
Объясняет стратегию: цель — **runtime compatibility**. Код должен запускаться без изменений в Node.js, Cloudflare Workers, Deno, Bun. Знает что Web Streams (`ReadableStream`) отличается от Node.js Streams API и умеет конвертировать между ними через `Readable.fromWeb()` / `Readable.toWeb()`. Понимает что это снижает vendor lock-in и упрощает edge-деплой.

---

### 13. ⭐⭐ Какие ключи используются при запуске Node.js?

**🟡 Junior**
Знает `--inspect` для дебага в Chrome DevTools.

**🟠 Middle**
Знает практические флаги:
```bash
node --max-old-space-size=4096 app.js  # лимит heap
node --inspect app.js                   # debugger
node --inspect-brk app.js              # пауза на старте
node --env-file=.env app.js            # загрузка .env (Node 20+)
node -r dotenv/config app.js           # preload (Node < 20)
```

**🔴 Senior**
Использует флаги профилирования и понимает что они дают:
```bash
node --prof app.js              # V8 CPU profiler → isolate-*.log
node --prof-process isolate-*.log  # читаемый отчёт
node --heap-prof app.js         # heap snapshot по завершении
node --trace-gc app.js          # логировать GC паузы
UV_THREADPOOL_SIZE=16 node app.js  # увеличить пул libuv
```
Знает когда увеличивать `--max-old-space-size` (утечки памяти vs реально большие данные) и чем это грозит (GC давление растёт с размером heap).

---

### 14. ⭐⭐⭐ Чем отличается node:cluster от node:child_process?
...
🔹 Блок 2: System Design + Ваш стек (25 мин)

### Перенесено из Блока 1:

#### ⭐⭐ Для чего нужны внутренние очереди и внешние MQ-системы?
...
### 2. Какие задачи может решать AI в Node.js-приложении?

**Level 2**
Называет базовые use cases:
- Чат-боты, ответы на вопросы
- Генерация текста (описания, summary)
- Классификация / тегирование

**Level 3**
Понимает более сложные паттерны:

| Паттерн | Пример |
|---|---|
| **RAG** (Retrieval-Augmented Generation) | Поиск по документам + ответ на основе найденного |
| **Function calling / Tools** | LLM вызывает ваши функции (поиск в БД, API) |
| **Structured output** | Модель возвращает JSON по схеме (`zod` + парсинг) |
| **Agents** | LLM сам решает какие шаги предпринять для достижения цели |
| **Embeddings** | Семантический поиск, рекомендации |

**Level 4**
Знает где AI не нужен и когда он вреден: детерминированные задачи (парсинг, валидация), real-time системы с жёсткими SLA, задачи где важна 100% точность без верификации.

---

### 3. Как безопасно интегрировать AI в Node.js? Кэширование промптов, fallback, observability.
